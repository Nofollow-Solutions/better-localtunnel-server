...
The above are important as the client will ask the server for a subdomain under a particular domain. The server will listen on any OS-assigned TCP port for client connections. You can limit the port range used by the better-localtunnel-server service by using the `client-min-port-range` and `client-max-port-range` cli parameters.
